...
import { getSettings, saveDisplaySettings } from '../storage/settings';
...
function Popup() {
  ...
  const checkConnection = async () => {
    ...
    const connected = await translationManager.testProvider(
      state.settings.translation.apiKey,
      state.settings.translation.model
    );
    setState(prev => ({ ...prev, isConnected: connected, isTesting: false }));
  };
  ...
  if (!state.settings) {
    return (
      <div className="container" style={{ textAlign: 'center', padding: '40px 16px' }}>
        ...
        <p style={{ marginTop: '12px', color: '#5f6368' }}>Loading...</p>
      </div>
    );
  }
    ...
    <div className="container">
      ...
      <div className="section">
        ...
        <div className="gemini-status">
          <div className={`status-dot ${state.isTesting ? '' : state.isConnected ? 'connected' : hasApiKey ? 'error' : ''}`} />
